...
The row-major or column-major convention also matters: the same buffer can give a different reshaped view depending on how linear memory is mapped to array indices.
...
3. Draw one possible `2 x 3` reshaped view using row-major order, and compare it with the first answer.
...
- When reworking Julia code in Rust, explicitly state how a flat buffer is mapped to multidimensional indices; a Rust `Vec` alone does not define row-major or column-major layout.
- For N-dimensional data, write down which index changes fastest before asking an AI agent to translate or optimize array code.
